...
void VolumetricFogEffect::UpdateSettings(){
	...
	if (TheShaderManager->GameState.isExterior) {
		...
		// interior point-light glow doesn't apply outdoors
		Constants.Glow.x = 0.0f;
		Constants.Glow.y = 0.0f;
	}
	else {
		...
		Constants.Glow.x = TheSettingManager->GetSettingF(SettingCategory, "InteriorGlowStrength");
		Constants.Glow.y = TheSettingManager->GetSettingF(SettingCategory, "InteriorGlowFadeRange");
	}
}
...
void VolumetricFogEffect::RegisterConstants(){
	...
	TheShaderManager->RegisterConstant("TESR_VolumetricFogGlow", &Constants.Glow);
}
