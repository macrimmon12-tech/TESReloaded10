// SpeedTree leaf VS -- BSSM_LEAVES_ADPt. Transcription of vanilla STLEAF003.vso.
// STLEAF001 plus one point light (colour c7, position+radius c12).
// vs_2_0 -> vs_3_0 to pair with the ps_3_0 shadow lookup.
//
// Engine uploads stop at c81, so Shadow.hlsl's c100/c104 are clear.
// Added: TEXCOORD3 sun-only, TEXCOORD4 shadowWorldPos. The point light stays in TEXCOORD1 and
// is never shadowed.
...
float4 PointPos      : register(c12);  // .w radius
...
float4 BranchRows[16] : register(c18);
...
struct VS_OUTPUT {
    float4 position       : POSITION;
    float2 uv             : TEXCOORD0;
    float4 lighting       : TEXCOORD1;   // vanilla combined: sun + ambient + point
...
VS_OUTPUT main(VS_INPUT IN) {
    ...
    // Point light, object space.
    float3 toPoint = PointPos.xyz - pos.xyz;
    float pointDist = length(toPoint);
    float pd = saturate(pointDist / PointPos.w);
    float3 pointLight = saturate(dot(N, toPoint / pointDist)) * PointColor.rgb * (1.0f - pd * pd);
    ...
    OUT.sun = sun;
    OUT.lighting = float4(sun + lightScale * AmbientColor.rgb + pointLight, 1.0f);
