...
namespace GrassNormals {
	...
	struct PropertyEntry {
		NiSourceTexture* texture;		// null: none found, and not looked for again
		bool matched;					// found as the texture bound for the draw, so revalidated against it
	};
	static std::unordered_map<UInt64, PropertyEntry> s_propertyTextures;	// (texturing, shade) property pair -> diffuse
	static std::unordered_map<NiSourceTexture*, TextureEntry> s_textures;
	...
	// Nothing here trusts a guessed structure layout: the grass texture is found by looking for an
	// object with NiSourceTexture's vtable, and accepted only if it is the texture bound for the draw.
	...
	static bool IsSourceTexture(const void* apObject) {
		__try {
			return apObject && *(const UInt32*)apObject == kVtblNiSourceTexture;
		}
		__except (EXCEPTION_EXECUTE_HANDLER) {
			return false;
		}
	...
	static IDirect3DBaseTexture9* D3DTextureOf(NiSourceTexture* apTexture) {
		__try {
			return apTexture->rendererData ? apTexture->rendererData->dTexture : nullptr;
		}
		__except (EXCEPTION_EXECUTE_HANDLER) {
			return nullptr;
		}
	...
	static const char* FilenameOf(NiSourceTexture* apTexture, char* apBuffer, UInt32 aSize) {
		__try {
			const char* name = apTexture->ddsPath1;
			if (!name) return nullptr;
			strncpy_s(apBuffer, aSize, name, _TRUNCATE);
			return name;
		}
		__except (EXCEPTION_EXECUTE_HANDLER) {
			return nullptr;
	...
	static bool LooksLikePointer(UInt32 aValue) {
		...
		return aValue >= 0x10000 && aValue < 0xFFFF0000 && !(aValue & 3);
	}

	// Depth-limited walk of an object's first words for NiSourceTexture objects: a property holds its
	// texture directly, or through an array of texture maps (property -> array -> map -> texture).
	// Prefers the one whose D3D texture is apBound; otherwise keeps the first found.
	static void Scan(const void* apObject, UInt32 aDepth, IDirect3DBaseTexture9* apBound, NiSourceTexture** apMatch, NiSourceTexture** apFirst) {
		const UInt32 width = aDepth == 3 ? 24 : 16;
		for (UInt32 i = 0; i < width && !*apMatch; i++) {
			UInt32 value = 0;
			__try {
				value = ((const UInt32*)apObject)[i];
			}
			__except (EXCEPTION_EXECUTE_HANDLER) {
				return;
			}
			...
			if (IsSourceTexture(candidate)) {
				NiSourceTexture* texture = (NiSourceTexture*)candidate;
				if (!*apFirst) *apFirst = texture;
				if (apBound && D3DTextureOf(texture) == apBound) *apMatch = texture;
				continue;
			}
			if (aDepth > 1) Scan(candidate, aDepth - 1, apBound, apMatch, apFirst);
		}
	}

	static NiSourceTexture* FindTexture(const NiPropertyState* apProperties, IDirect3DBaseTexture9* apBound) {
		const void* texturing = apProperties->m_spTextureProperty;
		const void* shade = apProperties->m_spShadeProperty;
		UInt64 key = ((UInt64)(UInt32)texturing << 32) | (UInt32)shade;
		...
		if (cached != s_propertyTextures.end()) {
			...
			// Revalidated every time, since a property freed with its cell can be reallocated at the
			// same address with another texture: still a texture, and still the bound one if it was.
			if (IsSourceTexture(entry.texture) && (!entry.matched || D3DTextureOf(entry.texture) == apBound))
				return entry.texture;
		}

		NiSourceTexture* match = nullptr;
		NiSourceTexture* first = nullptr;
		if (texturing) Scan(texturing, 3, apBound, &match, &first);
		if (!match && shade) Scan(shade, 3, apBound, &match, &first);
		NiSourceTexture* result = match ? match : first;

		if (s_propertyTextures.size() > 8192) s_propertyTextures.clear();
		s_propertyTextures[key] = { result, match != nullptr };
		if (result && !s_loggedFirst) {
			s_loggedFirst = true;
			char filename[MAX_PATH];
			Logger::Log("[GrassNormals] first grass texture found: %s (%s)", FilenameOf(result, filename, sizeof(filename)) ? filename : "?",
				match ? "the texture bound for the draw" : "not yet bound when constants are set; taken from the geometry");
		}
		if (!result && !s_loggedNoTexture) {
			s_loggedNoTexture = true;
			Logger::Log("[GrassNormals] could not find the texture of a grass geometry; its grass draws without a normal map. Not logged again.");
		}
		return result;
	}
	...
	static IDirect3DTexture9* NormalMapFor(NiSourceTexture* apTexture) {
		char filename[MAX_PATH];
	...
	static void Bind(const NiPropertyState* apProperties) {
		...
		if (grass->NormalMapStrength > 0.0f) {
			NiSourceTexture* texture = FindTexture(apProperties, renderState->GetTexture(0));
